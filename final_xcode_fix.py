...
def find_swift_files(directory):
    ...
    return sorted(swift_files, key=lambda x: x['relative_path'])
...
def find_existing_files(content):
    ...
    file_ref_pattern = r'/\* (.+?\.swift) \*/ = \{isa = PBXFileReference;.*?path = "?([^;"]+)"?;'
    matches = re.findall(file_ref_pattern, content, re.DOTALL)
    
    for match in matches:
        existing_files.add(match[0])  # Add filename
...
def determine_group_path(relative_path):
    """Determine the correct group/folder structure for a file"""
    # Map directory structure to groups
    ...
    if len(parts) == 1:
        ...
        # In subdirectory - use relative path
...
def add_files_to_project(content, swift_files):
    ...
    # Show first few files to be added for verification
    print("First 10 files to be added:")
    for i, file_info in enumerate(new_files[:10]):
        print(f"  {file_info['filename']} at {file_info['relative_path']}")
    ...
    for i, file_info in enumerate(new_files):
        ...
        # File reference entry with correct path
        path_declaration = determine_group_path(file_info['relative_path'])
        file_ref_entries.append(
            f"\t\t{file_ref_id} /* {file_info['filename']} */ = {{isa = PBXFileReference; lastKnownFileType = sourcecode.swift; {path_declaration} sourceTree = \"<group>\"; }};"
        )
...
def main():
    ...
    if os.path.exists(backup_file):
        print("Restoring backup project file...")
