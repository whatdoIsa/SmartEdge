...
enum AppError: LocalizedError, Identifiable, Equatable {
    ...
    var recoverySuggestion: String? {
        ...
        case .permissionsDenied:
            return "Go to System Preferences > Privacy & Security and grant required permissions"
        case .permissionsFailed:
            return "Try restarting the app or check system permissions"
        case .networkTimeout:
    ...
    static func == (lhs: AppError, rhs: AppError) -> Bool {
        switch (lhs, rhs) {
        case (.mediaServiceUnavailable, .mediaServiceUnavailable),
             (.notchServiceFailed, .notchServiceFailed),
             (.permissionDenied, .permissionDenied),
             (.permissionsDenied, .permissionsDenied),
             (.networkTimeout, .networkTimeout):
            return true
        case (.permissionsFailed(let lhsError), .permissionsFailed(let rhsError)):
            return lhsError.localizedDescription == rhsError.localizedDescription
        case (.initializationFailed(let lhsError), .initializationFailed(let rhsError)):
            return lhsError.localizedDescription == rhsError.localizedDescription
