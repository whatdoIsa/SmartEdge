...
struct MusicPlayerSettingsPanel: View {
    ...
    /// Track-change pulse controls. Surfaces the `notchPulseOnTrackChange`
    /// toggle (kept in the same section as music because it's specifically
    /// about music events) and a slider for the auto-collapse duration.
    /// The slider is disabled when the toggle is off so it visually
    /// reflects that the duration has no effect.
    private var pulseSettings: some View {
        VStack(alignment: .leading, spacing: 10) {
            Toggle("Pulse notch on track / play / pause change", isOn: $settings.notchPulseOnTrackChange)

            Text("Briefly open the notch when the song changes or you press play/pause, so you can see what's happening without hovering.")
                .font(.caption)
                .foregroundColor(.secondary)
            ...
            HStack {
                Text("Duration")
                    ...
                    .foregroundColor(.secondary)
                Slider(
                    value: $settings.notchPulseDurationSeconds,
                    in: 1.0...10.0,
                    step: 0.5
                )
